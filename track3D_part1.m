...
%       i=1; 
    ...
    dep1=depth_array;
    obj1=remove_bg(dep1, bg1, cmFromBG, farAwayObj, objPixelSize);
    ...
    dep2=depth_array;
    obj2=remove_bg(dep2,bg2, cmFromBG, farAwayObj, objPixelSize);
    ...
    if ~isempty(kn)
        ...
        for h=2:length(kn(1,:))
            DG=DG+sparse(kn(:,1),kn(:,h),D(:,h)<0.15,length(kn(:,1)),length(kn(:,1))); %up to 15cm
        end
        G=graph(DG,'upper');
        bins = conncomp(G);
        idx=bins;
